...
# Embedded file name: src\distancemarker\settings\config_param_types.py
...
class OptionsParam(Param):
    ...
    def renderParam(self, header, body=None, note=None, attention=None, showDefault=True):
        return {'type': 'Dropdown', 
           'text': header, 
           'varName': (self.tokenName), 
           'value': (self.defaultMsaValue), 
            'options': [{'label': (option.displayName)} for option in self.options], 'tooltip': (createTooltip(header=('%s (%s: %s)' % (header, Tr.DEFAULT_VALUE, self.getOptionByValue(self.defaultValue).displayName)) if showDefault else header, body=body, note=note, attention=attention)), 
           'width': 200}
